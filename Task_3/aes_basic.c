#include "algo_if.h"
#include "common.h"
#include <string.h>

/* ========== S‑box & 逆 S‑box ========== */
static const uint8_t sbox[256] = {
    
};
static const uint8_t inv_sbox[256] = {
    
};

static uint8_t xtime(uint8_t a) {
    return (uint8_t)((a << 1) ^ ((a & 0x80) ? 0x1b : 0x00));
}

static uint8_t mul(uint8_t a, uint8_t b) {
    uint8_t r = 0;
    while (b) {
        if (b & 1) r ^= a;
        a = xtime(a);
        b >>= 1;
    }
    return r;
}

static const uint32_t Rcon[10] = {
    0x01000000, 0x02000000, 0x04000000, 0x08000000,
    0x10000000, 0x20000000, 0x40000000, 0x80000000,
    0x1b000000, 0x36000000
};

/* ========== 密钥扩展 ========== */
static void aes_basic_key_schedule(const uint8_t *key, void *ctx) {
    uint32_t *W = (uint32_t*)ctx;
    for (int i = 0; i < 4; i++)
        W[i] = GETU32(key + 4 * i);

    for (int i = 4; i < 44; i++) {
        uint32_t temp = W[i - 1];
        if (i % 4 == 0) {
            uint32_t t = temp;
            temp = (sbox[(t >> 16) & 0xFF] << 24) |
                   (sbox[(t >>  8) & 0xFF] << 16) |
                   (sbox[ t        & 0xFF] <<  8) |
                   (sbox[ t >> 24 ]);
            temp ^= Rcon[i / 4 - 1];
        }
        W[i] = W[i - 4] ^ temp;
    }
}

/* ========== AddRoundKey：正确按列异或 ========== */
static void AddRoundKey(uint8_t state[16], const uint32_t *rk) {
    for (int col = 0; col < 4; col++) {
        uint32_t w = rk[col];
        state[4*col + 0] ^= (w >> 24) & 0xFF;
        state[4*col + 1] ^= (w >> 16) & 0xFF;
        state[4*col + 2] ^= (w >>  8) & 0xFF;
        state[4*col + 3] ^= w & 0xFF;
    }
}

static void SubBytes(uint8_t state[16]) {
    for (int i = 0; i < 16; i++) state[i] = sbox[state[i]];
}
static void InvSubBytes(uint8_t state[16]) {
    for (int i = 0; i < 16; i++) state[i] = inv_sbox[state[i]];
}

/* ShiftRows 保持不变（之前的实现基于列序正确） */
static void ShiftRows(uint8_t state[16]) {
    uint8_t tmp[16];
    tmp[0] = state[0];  tmp[1] = state[5];  tmp[2] = state[10]; tmp[3] = state[15];
    tmp[4] = state[4];  tmp[5] = state[9];  tmp[6] = state[14]; tmp[7] = state[3];
    tmp[8] = state[8];  tmp[9] = state[13]; tmp[10]= state[2];  tmp[11]= state[7];
    tmp[12]= state[12]; tmp[13]= state[1];  tmp[14]= state[6];  tmp[15]= state[11];
    memcpy(state, tmp, 16);
}
static void InvShiftRows(uint8_t state[16]) {
    uint8_t tmp[16];
    tmp[0] = state[0];  tmp[1] = state[13]; tmp[2] = state[10]; tmp[3] = state[7];
    tmp[4] = state[4];  tmp[5] = state[1];  tmp[6] = state[14]; tmp[7] = state[11];
    tmp[8] = state[8];  tmp[9] = state[5];  tmp[10]= state[2];  tmp[11]= state[15];
    tmp[12]= state[12]; tmp[13]= state[9];  tmp[14]= state[6];  tmp[15]= state[3];
    memcpy(state, tmp, 16);
}

/* ========== MixColumns：正确按列操作 ========== */
static void MixColumns(uint8_t state[16]) {
    for (int c = 0; c < 4; c++) {
        int idx = 4 * c;   // 列 c 的起始索引
        uint8_t a0 = state[idx+0], a1 = state[idx+1], a2 = state[idx+2], a3 = state[idx+3];
        state[idx+0] = xtime(a0) ^ (xtime(a1) ^ a1) ^ a2 ^ a3;
        state[idx+1] = a0 ^ xtime(a1) ^ (xtime(a2) ^ a2) ^ a3;
        state[idx+2] = a0 ^ a1 ^ xtime(a2) ^ (xtime(a3) ^ a3);
        state[idx+3] = (xtime(a0) ^ a0) ^ a1 ^ a2 ^ xtime(a3);
    }
}
static void InvMixColumns(uint8_t state[16]) {
    for (int c = 0; c < 4; c++) {
        int idx = 4 * c;
        uint8_t a0 = state[idx+0], a1 = state[idx+1], a2 = state[idx+2], a3 = state[idx+3];
        state[idx+0] = mul(0x0e, a0) ^ mul(0x0b, a1) ^ mul(0x0d, a2) ^ mul(0x09, a3);
        state[idx+1] = mul(0x09, a0) ^ mul(0x0e, a1) ^ mul(0x0b, a2) ^ mul(0x0d, a3);
        state[idx+2] = mul(0x0d, a0) ^ mul(0x09, a1) ^ mul(0x0e, a2) ^ mul(0x0b, a3);
        state[idx+3] = mul(0x0b, a0) ^ mul(0x0d, a1) ^ mul(0x09, a2) ^ mul(0x0e, a3);
    }
}

static void aes_basic_encrypt(const uint8_t *in, uint8_t *out, const void *ctx) {
    const uint32_t *rk = (const uint32_t*)ctx;
    uint8_t state[16];
    memcpy(state, in, 16);
    AddRoundKey(state, rk);
    for (int r = 1; r < 10; r++) {
        SubBytes(state);
        ShiftRows(state);
        MixColumns(state);
        AddRoundKey(state, rk + 4 * r);
    }
    SubBytes(state);
    ShiftRows(state);
    AddRoundKey(state, rk + 40);
    memcpy(out, state, 16);
}

static void aes_basic_decrypt(const uint8_t *in, uint8_t *out, const void *ctx) {
    const uint32_t *rk = (const uint32_t*)ctx;
    uint8_t state[16];
    memcpy(state, in, 16);
    AddRoundKey(state, rk + 40);
    for (int r = 9; r >= 1; r--) {
        InvShiftRows(state);
        InvSubBytes(state);
        AddRoundKey(state, rk + 4 * r);
        InvMixColumns(state);
    }
    InvShiftRows(state);
    InvSubBytes(state);
    AddRoundKey(state, rk);
    memcpy(out, state, 16);
}

const block_cipher_t aes_basic_impl = {
    .name = "AES-basic",
    .ctx_size = 176,
    .block_size = 16,
    .key_schedule = aes_basic_key_schedule,
    .encrypt = aes_basic_encrypt,
    .decrypt = aes_basic_decrypt
};
